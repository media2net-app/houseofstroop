...
async function main() {
  ...
  const coffeeCategory = await prisma.menuCategory.upsert({
    where: { id: 'coffee-drinks' },
    update: {},
    create: {
      id: 'coffee-drinks',
      title: 'Coffee & Drinks',
  ...
  const drinksShopCategory = await prisma.menuCategory.upsert({
    where: { id: 'drinks-shop' },
    update: {},
    create: {
      id: 'drinks-shop',
      title: 'Drinks & Shop',
  ...
  for (const sectionData of stroopwafelsSections) {
    ...
    for (let i = 0; i < sectionData.items.length; i++) {
      ...
      await prisma.menuItem.upsert({
        where: { id: `${sectionData.id}-item-${i}` },
        update: {},
        create: {
